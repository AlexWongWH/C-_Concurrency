...
//listing 2.7
// With this ownership able to create a number of threads and wait for them to finish
...
void do_work_f()
{
    ...
    //std::mem_fn, generates wrapper obj for pointers to members. To store and copy and involve mem to functions
...
int main()
{
    ...
    {
        // int ref {4};
        // func f{ref}; //creating a temp object with function call operator defined
        // std::thread t(f); //  
    ...
    //Bette to have threads containing local copy instead of pointers and reference
    //Have to note that if the main thread lifespan goes out of scope and it child is still running,

    //blocking, waiting for thread to complete -> calling join() or detach() 

    // //join() might be skipped if an exception is thrown, therefore have to join when exception was thrown
    {
